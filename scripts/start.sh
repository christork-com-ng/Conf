...
# Set custom webroot
if [ ! -z "$WEBROOT" ]; then
 sed -i "s#root /var/www/christork.com.ng;#root ${WEBROOT};#g" /etc/nginx/sites-available/default.conf
else
 webroot=/var/www/christork.com.ng
fi
...
if [ ! -d "/var/www/christork.com.ng/.git" ]; then
 ...
 if [ ! -z "$GIT_REPO" ]; then
   ...
   if [ ! -z ${REMOVE_FILES} ] && [ ${REMOVE_FILES} == 0 ]; then
     ...
     rm -Rf /var/www/christork.com.ng/*
   fi
   ...
   ${GIT_COMMAND} /var/www/christork.com.ng || exit 1
   if [ ! -z "$GIT_TAG" ]; then
   ...
   if [ -z "$SKIP_CHOWN" ]; then
     chown -Rf nginx.nginx /var/www/christork.com.ng
   fi
 fi
fi

# Enable custom nginx config files if they exist
if [ -f /var/www/christork.com.ng/conf/nginx/nginx.conf ]; then
  cp /var/www/christork.com.ng/conf/nginx/nginx.conf /etc/nginx/nginx.conf
fi

if [ -f /var/www/christork.com.ng/conf/nginx/nginx-site.conf ]; then
  cp /var/www/christork.com.ng/conf/nginx/nginx-site.conf /etc/nginx/sites-available/default.conf
fi

if [ -f /var/www/christork.com.ng/conf/nginx/nginx-site-ssl.conf ]; then
  cp /var/www/christork.com.ng/conf/nginx/nginx-site-ssl.conf /etc/nginx/sites-available/default-ssl.conf
fi
...
echo date.timezone=Africa/Lagos > /usr/local/etc/php/conf.d/timezone.ini
...
if [[ "$RUN_SCRIPTS" == "1" ]] ; then
  scripts_dir="${SCRIPTS_DIR:-/var/www/christork.com.ng/scripts}"
...
if [ -z "$SKIP_COMPOSER" ]; then
    ...
    if [ -f "/var/www/christork.com.ng/composer.lock" ]; then
        if [ "$APPLICATION_ENV" == "development" ]; then
            composer global require hirak/prestissimo
            composer install --working-dir=/var/www/christork.com.ng
        else
            composer global require hirak/prestissimo
            composer install --no-dev --working-dir=/var/www/christork.com.ng
        fi
    fi
fi
